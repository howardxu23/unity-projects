...
//            if (OnDetach != null) OnDetach.Invoke(this);
        ...
        public void OnClientReady(bool readyState)
        {
            if (readyState)
            {
                ...
                {
                    //writes player name to file and stores it locally
                    var playername = localPlayer.state.Name;

                    StreamWriter sw = new StreamWriter("PlayerData/player_name.txt");
                    sw.WriteLine(playername);//writes name to file
